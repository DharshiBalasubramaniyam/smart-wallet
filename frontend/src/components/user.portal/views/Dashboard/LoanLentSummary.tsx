...
function LoanLentSummary({ currency, loanSummary }: { currency: string, loanSummary: any }) {
    ...
    const interest = loanSummary?.interestReceived?.length > 0 ? loanSummary.interestReceived[0].amount : 0
    const principalReceived = loanSummary?.principalReceived?.length > 0 ? loanSummary.principalReceived[0].amount : 0;
    const rate = loanAmount > 0 ? (principalReceived / loanAmount * 100).toFixed(0) : 0;
    ...
    return (
        <section className="rounded my-3 p-3 border border-border-light-primary dark:border-border-dark-primary *:text-text-light-primary *:dark:text-text-dark-primary">
            ...
            <div className="flex gap-3 *:flex-1 flex-wrap">
                ...
                <div className="pb-2 border-border-light-primary dark:border-border-dark-primary mt-3">
                    <h1 className="font-semibold">Interest Received</h1>
                ...
                <div className="p-2 border border-border-light-primary dark:border-border-dark-primary mt-3">
                    ...
                    <div className="mt-3">
                        ...
                        <div className="mt-2 w-full bg-gray-200 rounded-full dark:bg-gray-700">
                            <div className="bg-primary text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full" style={{ width: `${Number(rate) > 100 ? 100 : rate}%` }}> {rate}%</div>
                        </div>
                    </div>
                </div>
                {/* <div className="p-2 border border-border-light-primary dark:border-border-dark-primary mt-3">
